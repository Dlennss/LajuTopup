#!/usr/bin/python3
"""Activate fixed LajuTopup staging paths; roll back both units if health checks fail."""
import fcntl
import os
...
BASE = Path('/var/lib/syslog-ng/fadlanpulsa/deployment/lajutopup')
STAGE = Path('/var/lib/syslog-ng/lajutopup')
...
with open('/run/lock/lajutopup-deploy.lock', 'w') as lock:
    ...
    binary = STAGE / 'pulsa-be/lajutopup-api'
    ...
    shutil.copyfile(binary, release / 'lajutopup-api')
    os.chmod(release / 'lajutopup-api', 0o755)
    (frontend / '.next/cache').mkdir(exist_ok=True)
    units = {
        'lajutopup-fe': '[Service]\nWorkingDirectory=' + str(frontend) + '\nBindPaths=\nBindPaths=/var/cache/lajutopup-fe:' + str(frontend / '.next/cache') + '\n',
        'lajutopup-be': '[Service]\nExecStart=\nExecStart=' + str(release / 'lajutopup-api') + '\n',
    }
    previous = {}
    try:
        ...
        run('systemctl', 'restart', 'lajutopup-be', 'lajutopup-fe')
        for attempt in range(30):
            try:
                fetch('http://127.0.0.1:8106/health')
                actual = fetch('http://172.22.0.1:33032/deploy-version.txt').strip()
                fetch('http://172.22.0.1:33032/')
                if actual == commit:
                    print('LajuTopup active: ' + commit, flush=True)
                    break
            except Exception:
                pass
            time.sleep(2)
        else:
            raise RuntimeError('LajuTopup health/release verification failed')
    except Exception:
