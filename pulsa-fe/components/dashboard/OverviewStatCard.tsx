...
function toneClass(tone: OverviewStatCardProps["tone"]): string {
  if (tone === "emerald") {
    return "border-emerald-200 border-l-[#bc4227] bg-[linear-gradient(135deg,#ffffff_0%,#f1fff8_100%)]";
  }
  if (tone === "amber") {
...
function iconClass(tone: OverviewStatCardProps["tone"]): string {
  if (tone === "emerald") return "bg-[#e8fff4] text-[#612f27] ring-emerald-300";
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3">
        <div className="text-xs font-black uppercase tracking-[0.12em] text-[#612f27]">{title}</div>
